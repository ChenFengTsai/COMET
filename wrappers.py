...
class MetaWorld:
    ...
    def step(self, action):
        ...
        return {'image': obs['image'], 'state': obs['state']}, obs['reward'], False, info
    ...
    def reset(self):
        ...
        return {'image': obs['image'], 'state': obs['state']}
...
class CollectDataset:
  ...
  def __init__(self, env, callbacks=None, precision=32, logger=None, mode='train'):
    self._env = env
    self._callbacks = callbacks or ()
    self._precision = precision
    self._episode = None
    self._mean_episode_score = 0
    self._mean_episode_success = 0
